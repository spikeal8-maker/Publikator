...
function calendarCard(item){return `<button type="button" class="calendar-card" data-calendar-post="${calEsc(item.id)}">${thumb(item)}<span class="calendar-card-main"><span class="calendar-card-title">${calEsc(timeLabel(item))} · ${calEsc(item.title)}</span><span class="calendar-card-meta">${calEsc(item.project_name)} · ${calEsc(platformLabel(item))}</span><span class="calendar-source">${calEsc(sourceLabel(item))} · ${calEsc(item.status)} · ${calEsc(item.schedule_timezone||'UTC')}</span></span></button>`;}
function listCard(item){return `<button type="button" class="calendar-list-card" data-calendar-post="${calEsc(item.id)}"><span class="calendar-time">${calEsc(timeLabel(item))}</span>${item.thumbnail_path?`<img src="/public-media/${calEsc(item.thumbnail_path)}" alt="">`:'<span class="calendar-thumb placeholder">нет</span>'}<span><strong>${calEsc(item.title)}</strong><span class="small muted" style="display:block">${calEsc(item.project_name)} · ${calEsc(platformLabel(item))}</span><span class="calendar-source">${calEsc(sourceLabel(item))} · schedule ${calEsc(item.schedule_timezone||'UTC')} · display ${calEsc(CALENDAR_DISPLAY_TIMEZONE)}</span></span><span class="calendar-statuses"><span class="badge ${calEsc(item.editorial_stage)}">${calEsc(item.editorial_stage)}</span><span class="badge ${calEsc(item.status)}">${calEsc(item.status)}</span></span></button>`;}
...
async function renderCalendar(){
  ...
  view.innerHTML=`<div class="calendar-shell"><div class="calendar-toolbar"><div class="calendar-toolbar-group"><button class="secondary" id="calendar-prev">←</button><button class="secondary" id="calendar-today">Сегодня</button><button class="secondary" id="calendar-next">→</button><span class="calendar-range-title">${calEsc(rangeTitle(range))}</span><span class="small muted">display: ${calEsc(CALENDAR_DISPLAY_TIMEZONE)}</span></div><div class="calendar-toolbar-group">${CALENDAR_MODES.map(mode=>`<button type="button" class="secondary calendar-mode ${mode===calendarMode?'active':''}" data-calendar-mode="${mode}">${MODE_LABELS[mode]}</button>`).join('')}</div></div><div id="calendar-content" class="card calendar-empty">Загрузка…</div></div>`;
  ...
  content.innerHTML=calendarMode==='month'?renderMonth(data.items,range):renderList(data.items,range);
  bindCalendarCards();
}
