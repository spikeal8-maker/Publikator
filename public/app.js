...
function vkDestinationSuffix(account){if(account?.platform!=='vk')return'';if(account.destination_kind==='PERSONAL')return' · личная страница';if(account.destination_kind==='COMMUNITY')return' · сообщество';return'';}
function targetsHtml(targets=[]){return `<h3>Площадки</h3><table class="table"><thead><tr><th>Площадка</th><th>Статус</th><th>Попытки</th><th>Ошибка</th><th></th></tr></thead><tbody>${targets.map(t=>`<tr><td>${esc(platformLabel(t.platform))} · ${esc(t.account_name)}${vkDestinationSuffix(t)}</td><td>${badge(t.state)}</td><td>${t.attempts}</td><td class="small error">${esc(t.last_error||'')}</td><td>${['FAILED','RETRY','RECOVERY_NEEDED'].includes(t.state)?`<button class="secondary retry-target" data-id="${t.id}">Повторить</button>`:''}</td></tr>`).join('')||'<tr><td colspan="5">Цели появятся после READY</td></tr>'}</tbody></table>`;}
async function projectsPage(){
  ...
  const targetsCell=(project)=>{
    ...
    return items.length?items.map((account)=>`<div>${esc(platformLabel(account.platform))} · ${esc(account.name)}${vkDestinationSuffix(account)}</div>`).join(''):'<span class="muted">Не выбраны</span>';
  };
