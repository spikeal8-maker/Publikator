...
async function postEditor(postId){
  ...
  const m=modal(`<h2>${post?'Публикация':'Новый пост'}</h2><form id="post-form" class="form-grid"><label>Проект<select name="projectId">${projects.map(p=>`<option value="${p.id}" ${post?.project_id===p.id?'selected':''}>${esc(p.name)}</option>`).join('')}</select></label><label>Режим<select name="scheduleMode"><option value="MANUAL">Вручную</option><option value="QUEUE" ${post?.schedule_mode==='QUEUE'?'selected':''}>Очередь</option><option value="AT" ${post?.schedule_mode==='AT'?'selected':''}>По дате</option></select></label><label class="full">Заголовок<input name="title" value="${esc(post?.title||'')}" required></label><label class="full">Текст<textarea name="body" required>${esc(post?.body||'')}</textarea></label><label class="full">Дата/время для AT<input name="scheduledAt" type="datetime-local"></label><div class="full"><strong>Изображения</strong><div class="media-list">${(post?.media||[]).map(x=>`<span><img src="/public-media/${x.relative_path}"><button type="button" class="secondary danger delete-media" data-id="${x.id}">Удалить</button></span>`).join('')}</div>${post?'<input id="media-file" type="file" accept="image/*">':'<div class="muted small">Сначала сохраните пост, затем добавьте изображение.</div>'}</div>${post?`<div class="full"><strong>Куда публиковать</strong><div class="target-picker">${post.targets.map(t=>`<label class="target-check"><input type="checkbox" name="accountId" value="${t.account_id}" ${t.enabled?'checked':''}> ${esc(t.platform)} · ${esc(t.account_name)}</label>`).join('')||'<span class="muted">Сначала подключите соцсеть</span>'}</div></div>`:''}<div class="full row-actions"><button class="primary" type="submit">Сохранить</button>${post?'<button type="button" id="mark-ready" class="secondary">Готов к публикации</button><button type="button" id="publish-now" class="secondary">Опубликовать сейчас</button>':''}<button type="button" id="close-modal" class="secondary">Закрыть</button></div></form><div id="post-error" class="error"></div>${post?targetsHtml(post.targets):''}`);
  m.querySelector('#close-modal').onclick=()=>m.remove();
  const form=m.querySelector('#post-form');
  const saveDraft=async()=>{const f=new FormData(form);const payload={projectId:f.get('projectId'),title:f.get('title'),body:f.get('body'),scheduleMode:f.get('scheduleMode'),scheduledAt:f.get('scheduledAt')||null};if(post){await api(`/api/posts/${post.id}`,{method:'PATCH',body:JSON.stringify(payload)});const accountIds=[...form.querySelectorAll('input[name="accountId"]:checked')].map(x=>x.value);await api(`/api/posts/${post.id}/targets`,{method:'PUT',body:JSON.stringify({accountIds})});return post.id;}const created=await api('/api/posts',{method:'POST',body:JSON.stringify(payload)});return created.id;};
  form.onsubmit=async e=>{e.preventDefault();try{await saveDraft();m.remove();await posts();}catch(err){m.querySelector('#post-error').textContent=err.message;}};
  if(post){
    ...
    m.querySelector('#mark-ready').onclick=async()=>{try{await saveDraft();await api(`/api/posts/${post.id}/ready`,{method:'POST'});m.remove();await posts();}catch(err){m.querySelector('#post-error').textContent=err.message;}};
    m.querySelector('#publish-now').onclick=async()=>{try{m.querySelector('#post-error').textContent='Публикация…';await saveDraft();await api(`/api/posts/${post.id}/publish-now`,{method:'POST'});m.remove();await posts();}catch(err){m.querySelector('#post-error').textContent=err.message;}};
    m.querySelectorAll('.delete-media').forEach(b=>b.onclick=async()=>{try{await api(`/api/media/${b.dataset.id}`,{method:'DELETE'});m.remove();await postEditor(post.id);}catch(err){m.querySelector('#post-error').textContent=err.message;}});
