...
import { previewGoogleSheetsCloudMedia } from './google-sheets-cloud-media.js';
...
export async function googleSheetsPollingTick(at: Date = new Date()): Promise<{
  ...
  if (pollRunning || maintenanceState().active) return { skipped: true, checked: 0, due: 0, previews: 0, warnings: 0, errors: 0 };
  pollRunning = true;
  const report = { skipped: false, checked: 0, due: 0, previews: 0, warnings: 0, errors: 0 };
  try {
    ...
    for (const connector of connectors) {
      ...
      try {
        ...
        event({
          level: 'error',
          type: POLL_FAILED,
          message: `Google Sheets preview poll failed: ${connector.name}: ${message}`,
          data: { connectorId: connector.id, intervalMinutes: connector.config.pollIntervalMinutes, error: message }
        });
      }
    }
    return report;
