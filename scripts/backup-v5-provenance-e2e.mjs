...
const restoredDb = new Database(config.dbPath, { readonly: true, fileMustExist: true });try {
  assert.equal(Number(restoredDb.pragma('user_version', { simple: true })), 6);
  const after = restoredDb.prepare(selectProvenance).get(postId);
  assert.deepEqual(after, before);
  console.log(JSON.stringify({ ok: true, schemaVersion: 6, canonicalBundle: true, pendingRestoreApplied: true, ingestionProvenancePreserved: true }, null, 2));
} finally {
  restoredDb.close();
