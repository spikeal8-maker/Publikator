...
const dataDir = await fs.mkdtemp(path.join(os.tmpdir(), 'publikator-browser-security-'));
process.env.NODE_ENV = 'test';
process.env.DATA_DIR = dataDir;
process.env.ADMIN_PASSWORD = 'browser-security-ci-password';
process.env.APP_MASTER_KEY = 'browser-security-master-key-that-is-longer-than-32-characters';
...
try {
  ...
  const hostFallbackMustNotOverrideConfiguredPublicOrigin = await app.inject({
    method: 'POST',
    url: '/api/projects',
    headers: { origin: 'http://local-admin.test:8080', host: 'local-admin.test:8080', cookie },
    payload: { name: 'Host bypass', slug: 'host-bypass' }
  });
  assert.equal(hostFallbackMustNotOverrideConfiguredPublicOrigin.statusCode, 403);
  ...
  // A trusted reverse proxy must keep login throttling isolated per forwarded client IP.
  for (let index = 0; index < 5; index += 1) {
    const response = await app.inject({
      method: 'POST',
      url: '/api/auth/login',
      ...injectedClient('198.51.100.10'),
  ...
  const otherClient = await app.inject({
    method: 'POST',
    url: '/api/auth/login',
    ...injectedClient('198.51.100.11'),
    payload: { password }
  });
  assert.equal(otherClient.statusCode, 200, otherClient.body);

  const blockedCount = Number(db.prepare("SELECT COUNT(*) AS count FROM projects WHERE slug IN ('must-not-exist','host-bypass')").get().count);
  assert.equal(blockedCount, 0);

  console.log(JSON.stringify({ ok: true, publicOrigin, securityHeaders: true, originGuard: true, trustedProxyRateLimitIsolation: true }, null, 2));
} finally {
  await app.close();
  db.close();
